import { findLatestAssistantMessage } from './history.mjs';
import { getRun, updateRun } from './runs.mjs';
import { sendOutboundEmail } from '../lib/agent-mail-outbound.mjs';
import { loadIdentity, loadOutboundConfig, updateQueueItem } from '../lib/agent-mailbox.mjs';
...
async function resolveAssistantReply(sessionId, run) {
  const assistantMessage = await findLatestAssistantMessage(sessionId, {
    includeBodies: true,
    match: (event) => {
...
export async function dispatchSessionCompletionTargets(session, run, options = {}) {
