...
> 当前发布基线：`v0.2` —— 这是继 `v0.1` 之后，第一个产品形态相对稳定的 RemoteLab 版本。
...
首轮对话完成后，RemoteLab 现在也可以让 agent 自动补齐会话标题、一级展示分组，以及一条隐藏的工作 description，让侧边栏保持分层感，但不再把分组和真实文件夹路径绑定在一起。
...
- agent 可以自动给相关会话做侧边栏分组，不需要再管理真实文件夹
- 发送消息——界面会通过 HTTP 重新拉取规范状态来收敛结果
- 关闭浏览器，过段时间回来——会话依然活着
- 可以直接粘贴截图到对话框
...
验证脚本现在统一放在 `tests/` 下，仓库根目录会更聚焦在运行入口和文档本身。
...
手机 ──HTTPS──→ Cloudflare Tunnel ──→ chat-server :7690
                                          ...
                                          HTTP 控制面
                                              │
                           持久化事件日志 + run 状态 + detached runners
                                              │
                             浏览器走 HTTP 读取，`/ws` 只负责提示刷新
```

### 会话持久化

session 元数据、规范化历史和每个 run 的状态都会落盘。浏览器始终通过 HTTP 读取规范状态，所以刷新 / 重连后会收敛到同一份真实状态，而不是依赖传输连续性。

活跃 run 现在在 detached sidecar 中执行。即使 `chat-server` 在运行中重启，控制面回来后也可以重新扫描 run 输出并恢复最终结果。`Resume` 仍然保留给明确被中断的 legacy / 手工恢复场景，只在抓到 Claude/Codex resume 元数据时出现。
...
如果你需要一份面向开发的系统总览和代码地图，请看 `docs/project-architecture.md`。

如果你想把邮件、GitHub、Bot 或其他外部工具接到同一套 session/message 流程上，请看 `docs/external-message-protocol.md`。
